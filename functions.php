...
// ---------------------------------------------------------------------
// 主题功能设置
function i_theme_settings() {
  // 公告功能设置
  add_option('i_notice_enabled', '0');
  add_option('i_notice_content', '');
  add_option('i_notice_cookie_time', '24');
  add_option('i_notice_always_show', '0');
  add_option('i_notice_last_modified', '0');
  
  // 广告位设置
  add_option('i_ads_article_bottom_enabled', '0');
  add_option('i_ads_article_bottom_code', '');
  add_option('i_ads_home_banner_enabled', '0');
  add_option('i_ads_home_banner_code', '');
}
add_action('admin_init', 'i_theme_settings');

// 判断是否显示公告
function should_show_notice() {
  if(get_option('i_notice_enabled') != '1') return false;
  
  $content = trim(get_option('i_notice_content'));
  if(empty($content)) return false;
  
  if(get_option('i_notice_always_show') == '1') return true;
  
  $last_modified = get_option('i_notice_last_modified', '0');
  if(isset($_COOKIE['notice_displayed'])) {
      $cookie_data = explode(':', $_COOKIE['notice_displayed']);
      $viewed_time = isset($cookie_data[0]) ? $cookie_data[0] : 0;
      $notice_version = isset($cookie_data[1]) ? $cookie_data[1] : 0;
      if($notice_version == $last_modified && $viewed_time > time()) {
          return false;
      }
  }
  return true;
...
// 显示广告的函数
function i_show_ad($position) {
  switch($position) {
      case 'article_bottom':
          if(get_option('i_ads_article_bottom_enabled') == '1') {
              echo '<div class="i-ad-wrap i-ad-article-bottom"><div class="i-ad-container">';
              echo wp_kses_post(get_option('i_ads_article_bottom_code'));
              echo '</div></div>';
          }
          break;
          
      case 'home_banner':
          if(get_option('i_ads_home_banner_enabled') == '1') {
              echo '<div class="i-ad-wrap i-ad-home-banner"><div class="i-ad-container">';
              echo wp_kses_post(get_option('i_ads_home_banner_code'));
              echo '</div></div>';
          }
          break;
  }
